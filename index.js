...
function indekseGoreFenomen(diziAdi, index) {
  return index + ". indekste bulunan fenomen: " + diziAdi[index].profile;
}
...
function profilListesi(diziAdi) {
  const dizi = [];

  for (let i = 0; i < diziAdi.length; i++) {
    dizi[i] = diziAdi[i].profile;
  }
...
function fenomenSil(diziAdi, indeks) {
  const yeniDizi = [];
  for (let i = 0; i < diziAdi.length; i++) {
    yeniDizi[i] = diziAdi[i];
  }
...
function fenomenEkle(diziAdi, a, b, c, d, e) {
...
function enFenomenler(diziAdi) {
  const yeniDizi = [];

  for (let i = 0; i < diziAdi.length; i++) {
    if (diziAdi[i].followers > 100000000) {
      yeniDizi.push(diziAdi[i].profile);
    }
  }
  return yeniDizi;
}
...
function fenomenGonderimSayisi(diziAdi, profilAdi) {
  for (let i = 0; i < diziAdi.length; i++) {
    if (diziAdi[i].profile == profilAdi) {
      return diziAdi[i].posts;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(diziAdi, platformAdi) {
  let eb = 0;
  let sonuc = "";
  for (let i = 0; i < diziAdi.length; i++) {
    if (diziAdi[i].platform == platformAdi {
      if (diziAdi[i].posts > eb) {
        eb = diziAdi[i].posts;
        sonuc=diziAdi.profile;
      }
    }
  }
  return sonuc;
